...
def getMovieName():  # function to get the movie name
    ...
    prompt = f"Create me 5 multiple-choice questions related to the movie {name} and mention its correct option for each question:\n"
    ...
    response = client.chat.completions.create( # getting the response from the model using the prompt
        ...
        messages=[{"role": "user", "content": prompt}, {
            "role": "user", "content": "Please ensure the response is in valid JSON"}], # ensuring the response is in valid json format to parse the data
        temperature=0.5,
        max_tokens=1024
    )
    print(response)
